...
import { chatAction, Message, ChatResult } from "@/app/actions/chat"
...
export function useChatBot(initialMessages: Message[] = []): UseChatBotResult {
  ...
  const createBotMessageFromResult = useCallback((result: ChatResult): Message => {
    return {
      type: "bot",
  ...
  const loadMore = useCallback(async (type: string) => {
    ...
    try {
      ...
      const cursor = cursorsRef.current[type as keyof typeof cursorsRef.current] || 0
      ...
      if (result[type as keyof ChatResult]?.length) {
        cursorsRef.current[type as keyof typeof cursorsRef.current] = cursor + 3
      }
