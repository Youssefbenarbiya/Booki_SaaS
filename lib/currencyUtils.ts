...
type CurrencyCode = "USD" | "TND" | "EUR" | "GBP" | "JPY" | "CAD" | "AUD"

// Define base currencies (currencies we have conversion rates for)
type BaseCurrencyCode = "USD" | "TND" | "EUR"

// Define exchange rates structure type
type ExchangeRatesType = {
  [key in BaseCurrencyCode]: {
...
const EXCHANGE_RATES: ExchangeRatesType = {
...
export async function convertCurrency(
  ...
  try {
    ...
    if (EXCHANGE_RATES[from as BaseCurrencyCode]?.[to]) {
      const result = amount * EXCHANGE_RATES[from as BaseCurrencyCode][to]!
      console.log(
        `Direct conversion: ${amount} ${from} = ${result} ${to} (rate: ${
          EXCHANGE_RATES[from as BaseCurrencyCode][to]
        })`
      )
      return result
    }
    ...
    if (EXCHANGE_RATES.USD[from] && EXCHANGE_RATES.USD[to]) {
      ...
      console.log(
        `USD conversion: ${amount} ${from} → ${amountInUSD} USD → ${result} ${to}`
      )
      console.log(
        `Rates used: ${from} to USD: ${
          1 / EXCHANGE_RATES.USD[from]!
        }, USD to ${to}: ${EXCHANGE_RATES.USD[to]!}`
      )
      return result
    }
    ...
    if (EXCHANGE_RATES.EUR[from] && EXCHANGE_RATES.EUR[to]) {
      ...
      console.log(
        `EUR conversion: ${amount} ${from} → ${amountInEUR} EUR → ${result} ${to}`
      )
      console.log(
        `Rates used: ${from} to EUR: ${
          1 / EXCHANGE_RATES.EUR[from]!
        }, EUR to ${to}: ${EXCHANGE_RATES.EUR[to]!}`
      )
      return result
    }
    ...
    if (EXCHANGE_RATES.TND[from] && EXCHANGE_RATES.TND[to]) {
      ...
      console.log(
        `TND conversion: ${amount} ${from} → ${amountInTND} TND → ${result} ${to}`
      )
      console.log(
        `Rates used: ${from} to TND: ${
          1 / EXCHANGE_RATES.TND[from]!
        }, TND to ${to}: ${EXCHANGE_RATES.TND[to]!}`
      )
      return result
    }
