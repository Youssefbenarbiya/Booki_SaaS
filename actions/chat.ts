...
export async function chatAction(
  ...
  try {
    ...
    switch (intent.type) {
      case "hotel_search":
        return await handleHotelSearch(intent, offset, limit);
      case "trip_search":
        return await handleTripSearch(intent, offset, limit);
      case "car_search":
        return await handleCarSearch(intent, offset, limit);
      case "room_availability":
        return await handleRoomAvailability(intent, offset, limit);
      case "booking_info":
        return await handleBookingInfo(intent, offset, limit);
      default:
        return {
          message:
            "I'm not sure how to help with that. Would you like to explore some options?",
          options: [
    ...
    return {
      message:
        "Sorry, I encountered an error while processing your request. Could you try again?",
...
async function handleHotelSearch(
  ...
  try {
    ...
    return {
      message:
        "Sorry, I couldn't search for hotels right now. Please try again later.",
...
async function handleTripSearch(
  ...
  try {
    ...
    if (intent.startDate) {
      ...
      conditions.push(
        gte(trips.startDate, intent.startDate.toISOString().split("T")[0])
      );
    }

    if (intent.endDate) {
      conditions.push(
        lte(trips.endDate, intent.endDate.toISOString().split("T")[0])
      );
    }
    ...
    if (tripsResult.length === 0) {
      return {
        message: `I couldn't find any trips${
          ...
          intent.startDate ? " for your selected dates" : ""
    ...
    return {
      message:
        "Sorry, I couldn't search for trips right now. Please try again later.",
...
async function handleCarSearch(
  ...
  try {
    ...
    return {
      message:
        "Sorry, I couldn't search for car rentals right now. Please try again later.",
...
async function handleRoomAvailability(
  ...
  try {
    ...
    // Apply pagination
    const rooms = await query.offset(offset).limit(limit);

    if (rooms.length === 0) {
      return {
        message:
          "I couldn't find any available rooms for your criteria. Would you like to try different dates?",
        options: [
          "Check next week",
          "Check next month",
          "Show me hotels instead",
    ...
    return {
      message:
        "Sorry, I couldn't check room availability right now. Please try again later.",
      options: ["Search for hotels", "Find a trip"],
    };
  }
}
...
async function handleBookingInfo(
  ...
  try {
    ...
    if (allBookings.length === 0) {
      return {
        message:
          "You don't have any bookings yet. Would you like to explore some options?",
        options: ["Show me hotels", "Show me car rentals", "Show me trips"],
      };
    }
    ...
    return {
      message:
        "Sorry, I couldn't retrieve your booking information right now. Please try again later.",
