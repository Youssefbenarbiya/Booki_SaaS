...
import { tripBookings, roomBookings, carBookings, trips, hotel, room, cars, user } from "@/db/schema";
...
export async function getBookingDetails(
  ...
  try {
    switch(type) {
      case "car":
        return await db.query.carBookings.findFirst({
          where: eq(carBookings.id, bookingId),
          with: {
            car: true
          }
      ...
      case "trip":
        return await db.query.tripBookings.findFirst({
          where: eq(tripBookings.id, bookingId),
          with: {
            trip: {
              with: {
      ...
      case "hotel":
        return await db.query.roomBookings.findFirst({
          where: eq(roomBookings.id, bookingId),
          with: {
            room: {
              with: {
...
export async function getAllBookings(type: BookingType) {
  ...
  if (!session?.user?.id || !["admin", "agency owner", "employee"].includes(session.user.role as string)) {
    throw new Error("Unauthorized access");
  }
  
  try {
    // For agency users, only show bookings for their own offerings
    const isAgency = session.user.role === "agency owner" || session.user.role === "employee";
    ...
    switch(type) {
      case "car":
        ...
        return await Promise.all(carResults.map(async (result) => {
      ...
      case "trip":
        ...
        const tripResults = await db
          ...
          .where(isAgency && agencyId ? eq(trips.agencyId, agencyId) : undefined);
        
        // Format the results to include trip details
        return await Promise.all(tripResults.map(async (result) => {
      ...
      case "hotel":
        ...
        const hotelResults = await db
          ...
          .where(isAgency && agencyId ? eq(hotel.agencyId, agencyId) : undefined);
        
        // Format the results to include hotel and room details
        return await Promise.all(hotelResults.map(async (result) => {
...
export async function updateBookingStatus(
  ...
  if (!session?.user?.id || !["admin", "agency"].includes(session.user.role as string)) {
    throw new Error("Unauthorized access");
  }
