...
// New function to get car details without requiring authentication
export async function getPublicCarById(id: number) {
  try {
    const car = await db.query.cars.findFirst({
      where: eq(cars.id, id),
      with: {
    ...
    console.error(`Failed to fetch public car with ID ${id}:`, error)
    throw new Error("Failed to fetch car")
  }
}
    ...
    if (discountPercentage !== undefined && discountPercentage !== null) {
      if (priceAfterDiscount === undefined || priceAfterDiscount === null) {
        priceAfterDiscount =
          ...
          (Number(data.originalPrice) * discountPercentage) / 100
      }
    }
