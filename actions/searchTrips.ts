...
export async function searchTrips(destination: string, startDate: string) {
  try {
    ...
    console.log("Using destination:", lowercaseDestination, "and startDate:", validStartDate)
    ...
    if (searchResults.length === 0) {
      console.log("No trips found with date constraint, trying without date filter")
      const fallbackResults = await db.query.trips.findMany({
        where: sql`LOWER(${trips.destination}) LIKE ${`%${lowercaseDestination}%`}`,
        with: {
          images: true,
          activities: true,
        },
